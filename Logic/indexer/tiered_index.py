...
class Tiered_index:
    ...
    def convert_to_tiered_index(
        ...
        for term, postings in current_index.items():
            term_frequency_sum = sum(postings.values())
            
            if term_frequency_sum >= first_tier_threshold:
                first_tier[term] = postings
            elif term_frequency_sum >= second_tier_threshold:
